...
open class CameraManager: NSObject, AVCaptureFileOutputRecordingDelegate, UIGestureRecognizerDelegate {
    ...
    open var focusMode : AVCaptureDevice.FocusMode = .locked
    
    /// Property to set exposure mode when tap to focus is used (_focusStart).
    open var exposureMode: AVCaptureDevice.ExposureMode = .locked
    ...
    func _changeExposureDuration(value: Float) {
        if (self.cameraIsSetup) {
            ...
            var newDurationSeconds = Float64(p * (maxDurationSeconds - minDurationSeconds)) + minDurationSeconds // Scale from 0-1 slider range to actual duration
            newDurationSeconds = 1/1000
            if (videoDevice.exposureMode == .custom) {
                var newExposureTime = CMTimeMakeWithSeconds(Float64(newDurationSeconds), preferredTimescale: 1000*1000*1000)
                newExposureTime = videoDevice.activeFormat.minExposureDuration
                var newIso = videoDevice.activeFormat.minISO
                videoDevice.setExposureModeCustom(duration: newExposureTime, iso: newIso, completionHandler: nil)
            }
        }
    }
    ...
    func configureCameraForHighestFrameRate(device: AVCaptureDevice) {
        ...
        for format in device.formats {
            for range in format.videoSupportedFrameRateRanges {
                if range.maxFrameRate == defaultParams.targetVideoFrameRate ?? 0 {
                    bestFormat = format
        ...
        if let bestFormat = bestFormat,
            ...
            do {
                ...
                // set our other settings for camera
                NSLog("%f %f", device.activeFormat.minISO, device.activeFormat.maxISO)
                device.setExposureModeCustom(duration: AVCaptureDevice.currentExposureDuration, iso: device.activeFormat.minISO, completionHandler: nil)
                ...
                //set white balance
                NSLog("max white balance")
                NSLog("%f", device.maxWhiteBalanceGain)
                device.setWhiteBalanceModeLocked(with: AVCaptureDevice.WhiteBalanceGains(redGain: device.maxWhiteBalanceGain, greenGain: 1.0, blueGain: 1.0), completionHandler: nil)
    ...
    fileprivate func _updateCameraQualityMode(_ newCameraOutputQuality: CameraOutputQuality) {
        // Get rid of this noise, we only allow the settings we want
